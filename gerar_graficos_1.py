...
# Função para salvar gráfico com HTML completo
def salvar_grafico_acessivel(fig, filename, titulo):
    """Salva o gráfico com HTML completo sem navbar e footer"""
    fig.update_layout(
        autosize=True,
...
df_total['Data Abertura'] = pd.to_datetime(
    df_total['Data Abertura'], errors='coerce')
df_total['Nova data'] = pd.to_datetime(
    df_total['Nova data'], errors='coerce')
...
fig1.update_layout(
    ...
    xaxis=dict(
        tickmode='array',
        tickvals=acidentes_por_ano.index.tolist(),
        ticktext=[str(a) for a in acidentes_por_ano.index.tolist()]
    )
)
salvar_grafico_acessivel(
    fig1, '01_total_acidentes_por_ano.html', 'Total de Acidentes por Ano'
)
...
fig2.add_trace(go.Bar(
    x=['Leve', 'Grave', 'Fatal'], y=victimas_2023.values,
    name='2023', marker_color=CORES_SIGMA[0]
))
fig2.add_trace(go.Bar(
    x=['Leve', 'Grave', 'Fatal'], y=victimas_2024.values,
    name='2024', marker_color=CORES_SIGMA[1]
))
fig2.add_trace(go.Bar(
    x=['Leve', 'Grave', 'Fatal'], y=victimas_2025.values,
    name='2025', marker_color=CORES_SIGMA[2]
))
fig2.update_layout(
...
salvar_grafico_acessivel(
    fig2, '02_vitimas_por_gravidade.html', 'Vítimas por Gravidade'
)
...
fig3.update_layout(
    ...
    xaxis=dict(
        tickmode='array',
        tickvals=vitimas_total.index.tolist(),
        ticktext=[str(a) for a in vitimas_total.index.tolist()]
    )
)
salvar_grafico_acessivel(
    fig3, '03_total_vitimas_por_ano.html', 'Total de Vítimas por Ano'
)
...
fig4 = make_subplots(
    rows=1, cols=3,
    subplot_titles=(
        'Top 10 Tipos - 2023', 'Top 10 Tipos - 2024', 'Top 10 Tipos - 2025'
    ),
...
salvar_grafico_acessivel(
    fig4, '04_tipos_acidentes.html', 'Tipos de Acidentes'
)
...
fig5 = make_subplots(
    rows=1, cols=3,
    subplot_titles=(
        'Top 15 Rodovias - 2023',
        'Top 15 Rodovias - 2024',
        'Top 15 Rodovias - 2025'
    ),
    specs=[[{'type': 'bar'}, {'type': 'bar'}, {'type': 'bar'}]]
)
fig5.add_trace(
...
salvar_grafico_acessivel(
    fig5, '05_rodovias_mais_acidentes.html', 'Rodovias com Mais Acidentes'
)
...
fig6.add_trace(go.Bar(
    x=regioes_2023.index, y=regioes_2023.values,
    name='2023', marker_color=CORES_SIGMA[0]
))
fig6.add_trace(go.Bar(
    x=regioes_2024.index, y=regioes_2024.values,
    name='2024', marker_color=CORES_SIGMA[1]
))
fig6.add_trace(go.Bar(
    x=regioes_2025.index, y=regioes_2025.values,
    name='2025', marker_color=CORES_SIGMA[2]
))
fig6.update_layout(
...
salvar_grafico_acessivel(
    fig6, '06_acidentes_por_regional.html', 'Acidentes por Regional'
)
print("✓ Gráfico 6 salvo")

# ===== GRÁFICO 7: OCORRÊNCIAS =====
# Normalizar os nomes de ocorrência (2025 usa capitalização diferente)
df['Ocorrencia_Norm'] = df['Ocorrencia'].str.title()  # Padroniza Title Case
...
ocorrencias_2023 = (
    df[df['Ano'] == 2023]['Ocorrencia_Norm']
    .value_counts()
    .reindex(top_ocorrencias, fill_value=0)
)
ocorrencias_2024 = (
    df[df['Ano'] == 2024]['Ocorrencia_Norm']
    .value_counts()
    .reindex(top_ocorrencias, fill_value=0)
)
ocorrencias_2025 = (
    df[df['Ano'] == 2025]['Ocorrencia_Norm']
    .value_counts()
    .reindex(top_ocorrencias, fill_value=0)
)
...
fig7.add_trace(go.Bar(
    x=top_ocorrencias, y=ocorrencias_2023.values,
    name='2023', marker_color=CORES_SIGMA[0]
))
fig7.add_trace(go.Bar(
    x=top_ocorrencias, y=ocorrencias_2024.values,
    name='2024', marker_color=CORES_SIGMA[1]
))
fig7.add_trace(go.Bar(
    x=top_ocorrencias, y=ocorrencias_2025.values,
    name='2025', marker_color=CORES_SIGMA[2]
))
fig7.update_layout(
...
salvar_grafico_acessivel(
    fig7, '07_tipos_ocorrencias.html', 'Tipos de Ocorrências'
)
